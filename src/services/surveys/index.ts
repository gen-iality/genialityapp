...
export const listenSurveysByEvent = (
  ...
  return firestore
    ...
    .onSnapshot(
      (snapshot) => {
        const surveys = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() } as SurveyInFirestore)
        );
...
export const changeSurveyStatus = async (
  ...
  try {
    ...
    if (
      Object.keys(status).includes("isOpened") &&
      status.isOpened !== undefined
    ) {
      if (status.isOpened) {
        statusToUpdated = {
    ...
    if (
      Object.keys(status).includes("isPublished") &&
    ...
    if (Object.keys(statusToUpdated).length) {
      const sendSurvey = async () => {
        const surveySave = await Promise.all([
          fireRealtime
            .ref(`events/general/surveys/${surveyId}`)
...
export const getQuorumByActivity = async (
  eventId: string,
  activityId: string
) => {
  ...
  const usersWhoHaveConnectedObject: Record<
    string,
    UsersWhoHaveConnected
  > | null = snapshot.val();

  if (!!usersWhoHaveConnectedObject) {
    const usersWhoHaveConnectedArray = Object.keys(
      usersWhoHaveConnectedObject
    ).map((userId) => ({
      id: userId,
      ...usersWhoHaveConnectedObject[userId],
    }));
    const usersWhoHaveConnectedQty = usersWhoHaveConnectedArray.length;
    const usersOnline = usersWhoHaveConnectedArray.filter(
    ...
    const totalAttendeesWeight = attendees.reduce((acc, attendee) => {
      if (attendee.properties.voteWeight) {
        acc += attendee.properties.voteWeight
          ? Number(attendee.properties.voteWeight)
          : 1;
      } else {
        acc += 1;
      }
    ...
    if (!attendeesState.weight || !totalAttendeesWeight) {
      ...
      const quorum = numberDecimalToTwoDecimals(
        (attendeesState.weight / totalAttendeesWeight) * 100
      );
      return quorum;
    }
  }
...
const getAttendeesByEvent = async (eventId: string) => {
  ...
  if (snapshot.empty) {
    ...
    const attendeesSnapshot = snapshot.docs.map(
