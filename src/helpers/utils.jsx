...
export async function parseData2Excel(data, fields, roles = null) {
	...
	data.map((item, key) => {
		...
		info[key]['Hora checkIn'] = item.checkedin_at ? moment(checkedInAt).format('DD/MM/YYYY H:mm:ss A') : '';
...
export const convertFormat = (text) => {
	// Convert to lowercase, remove accents, and replace spaces with hyphens
	const result = removeAccents(text)
		.toLowerCase()
		.replace(/\s+/g, '-');
	return result;
};
