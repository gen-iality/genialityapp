...
export const fireStoreApi = {
  createOrUpdate: (eventId, activityId, eventUser) => {
    const agendaRef = firestore.collection(`event_activity_attendees/${eventId}/activities/${activityId}/attendees`);
...
export const EventsApi = {
  ...
  getcurrentUserEventUser: async (event_id) => {
    const token = await GetTokenUserFirebase();
    const response = await Actions.getAll(`/api/me/eventusers/event/${event_id}?token=${token}`, false);
...
export const Activity = {
  ...
  checkInAttendeeActivity: async (event_id, activity_id, user_id) => {
    ...
    const result = await Actions.put(`api/events/${event_id}/activities_attendees/checkin?token=${token}`, data);
    return result;
  },
