...
export const disabledStartDate = (
  ...
  userConsumption: userConsumptionProps,
) => {
  ...
  if (userConsumptionEndDate) {
    aditionalDays = dayjs(userConsumptionEndDate).add(1, 'day').diff(startDate, 'days')
  }
...
export const disabledEndDate = (
  ...
  const addExtraTime = dayjs(startDate).add(
    streamingHours - equivalentToADayInMinutes,
    'minutes',
  )
...
const disableMinutesRange = (event: eventProps, streamingHours: number) => {
  ...
  for (
    ...
    initialMinutes < dayjs(hour_start).minute();
    initialMinutes++
  ) {
