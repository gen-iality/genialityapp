...
const initBroadcastViewers = async (
  ...
  if (!userContext.value) {
    ...
    if (userAnonimo) {
      ...
      const refViewer = fireRealtime.ref(
        `/viewers/${event_id}/activities/${activity_id}/uniqueUsers/${user._id}`,
      )
...
const initUserPresenceInner = async (
  user: any,
  event_id: string,
  activity_id: string,
  state?: string,
  ...
  const uniqueUsers = fireRealtime.ref(
    `/viewers/${event_id}/activities/${activity_id}/uniqueUsers/${uid}`,
  )
  const totalViews = fireRealtime.ref(
    `/viewers/${event_id}/activities/${activity_id}/total/${time}`,
  )
