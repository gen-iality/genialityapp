...
export const useGetOrganizationUsersOnlyName = (organizationId: string) => {
  ...
  const getOrgUser = useCallback(async (): Promise<{ error: boolean; data: any[]; meta?: any }> => {
    try {
      const response = await OrganizationApi.getUsersOnlyName(organizationId);
  ...
  const getDataOrgUser = useCallback(async () => {
    try {
      ...
      let otherOrgUser = [];
      if (meta) {
        otherOrgUser = await getOrgUserAllPages(meta.last_page);
      }
