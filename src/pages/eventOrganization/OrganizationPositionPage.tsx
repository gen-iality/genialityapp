...
/** Antd imports */
import { Table, Row, Col, Button, Typography, Spin, Modal, Form, Select, Tooltip, Tag } from 'antd';
import { DeleteOutlined, PlusCircleOutlined } from '@ant-design/icons';
import type { ColumnsType } from 'antd/lib/table';

/** Helpers and utils */
import { PositionsApi, EventsApi, OrganizationApi } from '@helpers/request';
...
function OrganizationPositionPage(props: OrganizationPositionPageProps) {
  ...
  const addOrgUserToPosition = async (orgUserId: string) => {
    const result = await PositionsApi.Organizations.addUser(organizationId, positionId, orgUserId);
  ...
  useEffect(() => {
    ...
    Promise.all(allEventIds.map(async (eventId: string) => await EventsApi.getOne(eventId))).then((events: any[]) =>
      ...
      },
      {
        ...
        render: (orgUser: any) => (
          <Link to={`${props.match.url}/user/${orgUser.account_id}`}>
            <CertificationTag value={orgUser.user.certifications.length} total={allPositionEvents.length}>
  ...
  return (
    <>
      ...
      />
      <Modal visible={isModalOpened} title="Agrega miembro" onCancel={closeModal} onOk={() => form.submit()}>
        <Typography.Paragraph>Agrega miembros de la organización a este cargo.</Typography.Paragraph>
        <Typography.Paragraph>Hay {allOrgUsers.length} usuarios que pueden ser agregados.</Typography.Paragraph>
        ...
        <Form
          ...
          onFinish={(values: any) => {
            ...
            setIsLoading(true);
            Promise.all(
              values.orgUsers.map(async (userId: string) => {
