...
import { Upload, Input, Tabs, Button, Form } from 'antd'
...
const PositionCertificationFileUploader: React.FunctionComponent<
  IPositionCertificationFileUploaderProps
> = (props) => {
  const {
    value,
    path: targetPath,
  ...
  const onCustomRequest: UploadProps['customRequest'] = (options) => {
    ...
    // @ts-expect-error
    const filename =
      dayjs().format('YYYY-DD-MM HH:mm:ss') + '-' + (file.name || 'unnamed')
  ...
  useEffect(() => {
    if (value) {
      ...
      if (!currentFirebasePath.startsWith('documents/')) {
        console.warn(
          'This url seems to be from non-Firebase URL, but it is not a problem',
        )
      } else {
          ...
          <Upload
            ...
            onRemove={(file) => {
              ...
              if (firebasePath) {
                ...
                onChange(undefined)
                console.warn(
                  'If this URL is from Firebase, then it is a problem. But, if it is from another URL there is no problem :)',
                )
              }
