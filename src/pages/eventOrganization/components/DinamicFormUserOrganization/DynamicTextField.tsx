...
const DynamicTextField: React.FunctionComponent<IDynamicTextFieldProps> = (props) => {
  ...
  const isHiddenField = useMemo(
    () => (allInitialValues?.email && name === 'email') || (allInitialValues?.names && name === 'names') || false,
    [allInitialValues]
  );
  ...
  return (
    <DynamicFormItem fieldData={fieldData} rules={rules} hidden={isHiddenField} initialValue={allInitialValues[name]}>
      <Input
        {...secondProps}
        addonBefore={
