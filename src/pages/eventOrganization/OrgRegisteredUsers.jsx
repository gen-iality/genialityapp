...
function OrgRegisteredUsers(props) {
  ...
  useEffect(() => {
    getRegisteredUsers()
    setExtraFields(
      props.org.user_properties.filter((item) => !['email', 'names'].includes(item.name)),
    )
  }, [isSubmiting])
    ...
    const totalRegistered = (
      await Promise.all(
        orgEvents.map(async (orgEvent) => {
          ...
          const certificationsByEvent = await CerticationsApi.getByUserAndEvent(
            null,
            orgEvent._id,
          )
          ...
          const registeredByEvent = querySnapshot.docs.map((doc) => {
            ...
            const filteredDynamicField = (
              orgEvent.organiser.user_properties || []
            ...
            const userCertification = certificationsByEvent.find(
              (certificationByEvent) =>
                certificationByEvent.user_id === infoEventUser?.account_id,
            )
            if (userCertification?.approved_until_date) {
              properties.validity_date = dayjs(userCertification.approved_until_date)
              properties.approved_until_date = dayjs(
                userCertification.approved_until_date,
              )
  ...
  const addNewCertificationModal = (item) => {
    ...
    EventsApi.getOne(item.event_id)
      .then((eventData) => {
        /**
  ...
  const onFormFinish = (values) => {
    ...
    values = {
      ...values,
      user_id: selectedOrgMember.account_id,
      event_id: selectedOrgMember.event_id,
    }
