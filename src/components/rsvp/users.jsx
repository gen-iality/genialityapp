...
class UsersRsvp extends Component {
    ...
    async componentDidMount() {
        try {
            ...
            columns.unshift(<div className="field">
    ...
    //Dropdowns
    handleDropUser = () => this.setState(prevState=>{return {dropUser:!prevState.dropUser}});
    handleDropSend = () => this.setState(prevState=>{return {dropSend:!prevState.dropSend}});
    ...
    render() {
        ...
        return (
            <Fragment>
                                        ...
                                        <div className={`dropdown is-pulled-right is-right ${dropUser?"is-active":""}`} onClick={this.handleDropUser}>
                                            ...
                                            <div className="dropdown-menu" id="dropdown-menu1" role="menu">
                                                <div className="dropdown-content">
                                                    <div className="dropdown-item" onClick={this.modalUser}><p>Nuevo usuario</p></div>
                                                    <Link className="dropdown-item" to={`${this.props.matchUrl}/importar-excel`}>Importar usuarios de Excel</Link>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <p>Seleccionados: <strong> {this.state.auxArr.length}</strong></p>
                                {
                                    this.state.auxArr.length>0&&<div className={`dropdown ${dropSend?"is-active":""}`} onClick={this.handleDropSend}>
                                        <div className="dropdown-trigger">
                                            <button className="button" aria-haspopup aria-controls={"dropdown-menu"}>
                                                <span>Enviar</span>
                                                <span className="icon"><i className="fas fa-angle-down"/></span>
                                            </button>
                                        </div>
                                        <div className="dropdown-menu" id="dropdown-menu" role="menu">
                                            <div className="dropdown-content">
                                                <div className="dropdown-item"><p>Invitaciones</p></div>
                                                <div className="dropdown-item"><p>Correos</p></div>
                                                <div className="dropdown-item"><p>Entradas</p></div>
                                            </div>
                                        </div>
                                    </div>
                                }
                                <EvenTable head={columns}>
                                    {
                                        pageOfItems.map(user=>(
                                            <tr key={user.id}>
                                                <td>
                                                    <div>
                                                        <input className="event-inv-check is-checkradio is-small" id={"checkinUser"+user.id}
                                                               type="checkbox" name={"checkinUser"+user.id} checked={this.isChecked(user.id)} onChange={(e)=>{this.toggleSelection(user)}}/>
                                                        <label htmlFor={"checkinUser"+user.id}/>
                                                    </div>
                                                </td>
                                                {Object.keys(user.properties).map(prop=><td key={prop}>{user.properties[prop]}</td>)}
                                            </tr>
                                        ))
                                    }
                            </EvenTable>
                ...
                {this.state.addUser &&
                <AddUser handleModal={this.closeModal} modal={this.state.addUser} eventId={this.props.eventID}
