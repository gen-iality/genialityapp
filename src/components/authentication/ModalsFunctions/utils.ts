export const FrasesInspiradoras = [
    'Para lograr grandes cosas, no solo debemos actuar, también debemos soñar, no solo es necesario planear, también debemos creer. – Anatole France',
    'El éxito es el resultado de la perseverancia, de la persistencia y de la esperanza. – Henri Matisse',
    'El éxito no es el final de la carrera, sino el comienzo de la siguiente. – Henry Ford',
    'El éxito no es el resultado de los esfuerzos, sino del entusiasmo. – Vince Lombardi',
    'El éxito no es el resultado de muchas oportunidades, sino de un solo esfuerzo. – Thomas J. Watson',
    'El éxito no es el resultado de muchas oportunidades, sino de un solo esfuerzo. – Thomas J. Watson',
    'Cuando dejas de perseguir las cosas negativas, le das la oportunidad a las cosas positivas para que te alcancen. – Lolly Daskal',
    'Recuerda que no puedes fallar en ser tú mismo. – Wayne Dyer',
    'Haz lo que puedas, con lo que tengas, donde estés. – Theodore Roosevelt',
    'Con un solo nada es suficiente. – David Borja',
];
