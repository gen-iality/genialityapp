...
function calcTime(item: any) {
  ...
  if (!item.endTimestamp)
    return dayjs.unix(item.startTimestamp / 1000).format('YYYY-MM-DD HH:mm:ss')
...
export function PresenceListPage(props: IPresenceListPageProps) {
  ...
  return (
    <div>
      <Button danger onClick={onDeleteAllLogs}>
        Delete all logs
      ...
      <List
        ...
        renderItem={(item, index) => (
          <List.Item key={index}>
            ...
            <Typography.Text style={{ backgroundColor: item[1] ? '#33984A' : '#C53840' }}>
              {JSON.stringify(item[1])}
            </Typography.Text>
            <Button danger onClick={() => onDeleteBeacon(item[0])}>
              <DeleteOutlined />
            </Button>
          </List.Item>
        )}
