...
export const useDrawerBingo = () => {
  ...
  const [ballotValue, setBallotValue] = useState<PickedNumberInterface>({
    type: '',
    value:
  ...
  const [dataFirebaseBingo, setDataFirebaseBingo] = useState<BingoGame | null>(null);
  ...
  useEffect(() => {
    if (dataFirebaseBingo) {
      setBallotValue(
        dataFirebaseBingo.currentValue ?? {
          type: '',
          value:
            '¡El bingo aún no ha iniciado! Acá puedes encontrar tu cartón virtual para que interactúes con el y todas opciones. Si deseas puedes imprimir tu cartón oprimiendo el botón "Descargar Cartón"',
        }
