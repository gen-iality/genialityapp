  ...
  formatAttendeesForTable(attendees) {
    ...
    for (let i = 0; attendees.length > i; i++) {
      if (attendees[i].properties) {
        ...
        attendeeFlattenedData.ticket = attendees[i]?.ticket
          ? attendees[i]?.ticket?.title
          : ''
        attendeeFlattenedData.checkedin_at = attendees[i]?.checkedin_at
          ? attendees[i]?.checkedin_at
          : ''
    ...
    for (let i = 0; propertiesTable?.length > i; i++) {
      columnsTable.push({
        title: propertiesTable[i].label
          ? propertiesTable[i].label
          : propertiesTable[i].name,
        dataIndex: propertiesTable[i].name,
        /* width: '300', */
        ellipsis: true,
        sorter: (a, b) =>
          a[propertiesTable[i].name]?.length - b[propertiesTable[i].name]?.length,
              ...
              <Col>
                <Button
                  ...
                  onClick={() => {
                    ...
                    const promises = attendeesFormatedForTable.map(async (user) => {
                      const { email } = user
