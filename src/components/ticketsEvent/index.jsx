...
let Tickets = (props) => {
  ...
  function newTicket() {
    ...
    if (!tickets.find(({ _id }) => _id === 'new')) {
      setTickets(
        (ticket = tickets.concat({ title: '', allowed_to_vote: allowed_to_vote, created_at: new Date(), _id: 'new' }))
      );
      setId('new');
      ...
      <EventContent title={'Tickets'} addAction={newTicket} addTitle={'Nuevo ticket'}>
        <EvenTable head={['Id', 'Nombre', 'Permiso de Voto', 'Fecha Creación', 'Acciones']}>
          {tickets.map((ticket, key) => {
            return (
              <tr key={key}>
                <td>{ticket._id !== id && <p>{ticket._id}</p>}</td>
                <td>
                  {ticket._id !== id ? (
                    <p>{ticket.title}</p>
                  ) : (
                ...
                <td>
                  {ticket._id !== id ? (
                    ...
                    <select
                      defaultValue={false}
                      value={allowed_to_vote}
                      autoFocus
                      onChange={(e) => setAllowedToVote(e.target.value)}>
