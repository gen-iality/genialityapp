...
function Presence<T = any>(props: PresenceProps<T>) {
  ...
  const { debuglog: LOG = console.debug, errorlog: ERROR = console.error } = props
  /* eslint-enable no-console */

  const { realtimeDB, global: isGlobal, data } = props
  ...
  const sessionHandler = async (isConnected: boolean) => {
    if (isConnected) {
      ...
      if (isGlobal) {
        userSessionsRealtime = realtimeDB
          .ref(`/user_sessions/global`)
          .child(props.collectionId)
          .push()
      } else {
        userSessionsRealtime = realtimeDB
          .ref(`/user_sessions/local`)
          .child(props.collectionId)
          .push()
      }
      ...
      if (isGlobal) {
        ...
        if (!beacon) {
          ...
          try {
            ...
            ERROR(
              'tried set a value in disconnection for userSessionsRealtimeBlocker:',
              err,
            )
          }
