...
import { Row, Col, Form, Input, message, Modal, Select, Card } from 'antd';
...
const Certificado = (props) => {
  ...
  useEffect(() => {
    if(locationState.edit) {
      ...
      setCertificado({...certificado, content: initContent})
    }
    getRol();
  }, []);
  ...
  const onSubmit = async () => {
    ...
    try {
      if(locationState.edit) {
        await CertsApi.editOne(certificado, locationState.edit);
      } else {
        await CertsApi.create(certificado);
      }     
        ...
        <Col span={24}>
          ...
          <Form.Item label={'Vista Previa'}>
            <div className='editor-certificado'>
              <div className='contenedor' >
                <img src={imageFile} alt='background-cert' className='bg' />
                <div style={{textAlign: 'center'}} dangerouslySetInnerHTML={{ __html: certificado.content }}/>
              </div>
            </div>
