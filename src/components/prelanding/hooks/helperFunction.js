...
export const obtenerData = async (cEvent) => {
  ...
  const agendaConfigFilter=agendaConfig.filter((agendaCfg)=>agendaCfg.isPublished || agendaCfg.isPublished==undefined)
  return {
...
export const obtenerConfigActivity=async (event,agenda)=>{
 const listAgenda=[];
  if(agenda){
    await Promise.all(agenda.map(async(activity)=>{     
