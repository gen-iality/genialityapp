const printBagde = (ifrmPrint, badge) => {
  console.log('🚀 ~ file: printBagde.jsx ~ line 2 ~ printBagde ~ badges', badge);
  ...
  for (; i < badge.length; ) {
    if (badge[i].line) {
      if (badge[i].qr) oDoc.write(`<div><img src=${qr}></div>`);
        ...
        oDoc.write(
          `<p style="font-family: Lato, sans-serif;font-size: ${badge[i].size}px;text-transform: uppercase">${badges[i].id_properties.value}</p>`
        );
      i++;
    } else {
      if (badge[i + 1] && !badge[i + 1].line) {
        ...
        if (!badge[i].qr) {
          oDoc.write(
            `<p style="font-family: Lato, sans-serif;font-size: ${badge[i].size}px;text-transform: uppercase">${badges[i].name}]}</p>`
          );
        } else {
          oDoc.write(`<div><img src=${qr}></div>`);
        }
