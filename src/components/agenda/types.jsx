...
class AgendaTypes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading:true,
    ...
    async componentDidMount() {
        ...
        const list = await TypesAgendaApi.byEvent(this.eventID);
    ...
    async saveRole() {
        try{
            if(this.state.id !== 'new') {
                await TypesAgendaApi.editOne({name: this.state.name}, this.state.id, this.eventID);
                this.setState(state => {
                ...
                const newRole = await TypesAgendaApi.create(this.eventID, {name: this.state.name});
                this.setState(state => {
                    const types = state.list.map(item => {
    ...
    async removeType(deleteID) {
        ...
        try {
            await TypesAgendaApi.deleteOne(deleteID);
