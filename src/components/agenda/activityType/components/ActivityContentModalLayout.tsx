...
function ActivityContentModalLayout(props: ActivityContentModalLayoutProps) {
  ...
  const handleGoForward = () => {
    ...
    if (selected) {
      ...
      const transmissions: ActivityType.GeneralTypeValue[] = ['RTMP', 'eviusMeet']
  ...
  const isTimeToCreate = useMemo(() => {
    ...
    if (
      'widgetType' in widgetData &&
      (widgetData.widgetType === WidgetType.FINAL ||
        widgetData.widgetType === WidgetType.FORM)
    )
      return true
