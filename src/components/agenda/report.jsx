...
class ReportList extends Component {
    ...
    componentDidMount() {
        ...
        console.log("evento por props", event)
        let days = [];
    ...
    render() {
        ...
        const columns = [
            {
                title: 'Hora',
                render: text => (<a>{Moment(text.datetime_start, "YYYY-MM-DD HH:mm").format("HH:mm")} - {Moment(text.datetime_end, "YYYY-MM-DD HH:mm").format("HH:mm")}</a>),
            },
            {
                title: 'Actividad',
                dataIndex: 'name',
            },
            {
                title: 'Action',
                render: text => (
                    <Link to={{ pathname: `${this.props.url}/checkin/${text._id}`, state: { name: text.name, id: text._id, event: this.props.event } }}>
                        <RightOutlined />
                    </Link>
                )
            }
            ...
            <EventContent title={"CheckIn por Actividad"} classes={"agenda-list"}>
                <nav className="level">
                    <div className="level-left">
                        {
                            days.map((date, key) => <div onClick={() => this.selectDay(date)} key={key} className={`level-item date ${Moment(date).format("MMMM-DD") === Moment(day).format("MMMM-DD") ? "active" : ""}`}>
