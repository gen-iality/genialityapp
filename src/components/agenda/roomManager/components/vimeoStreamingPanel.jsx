...
const VimeoStreamingPanel = ({ meeting_id, activityEdit }) => {
  ...
  let linkAdmin =
    eviusmeets + `?meetingId=${activityEdit}&rol=1&username=${names}&email=${email}&photo=${picture ? picture : ''}`;
      ...
      case 'StreamYard':
        return (
          <Button type='primary' href={streamYardUrl} target='_blank'>
            {transmition === 'StreamYard' && `Configurar transmisión en StreamYard`}
          </Button>
        );
      case 'EviusMeet':
        return (
              ...
              <Col span={14}>
                <Tabs defaultActiveKey='1'>
                  <Tabs.TabPane tab={'Datos'} key='1'>
                    ...
                    <b>{transmition}</b>
                    <Typography.Text>
                      <Space>{/* <b>{livestreamStatus?.state}</b> */}</Space> <br />
                    </Typography.Text>
                    <Typography.Text type='secondary'>
                      {/* Origin Conectado: */}
                      <br />
