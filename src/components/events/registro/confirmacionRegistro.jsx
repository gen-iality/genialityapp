...
function ConfirmacionRegistro(props) {
  ...
  const saveData = async () => {
    let data = {
      registration_message: event.registration_message,
      validateEmail: validateEmail ? validateEmail : event.validateEmail,
      send_custom_email: sendCustomEmail ? sendCustomEmail : event.send_custom_email
    };
    ...
    <>
      ...
      <Row gutter={[0, 24]}>
        <Col span={12}>
          ...
          <ReactQuill value={event.registration_message} onChange={chgTxt} />
        </Col>
      </Row>
      <Row>
        {/* Se envia el valor a validEmail a useSate para usarla porteriormente en la funcion saveData */}
        <Checkbox
          defaultChecked={event.validateEmail}
          style={{ marginRight: '2%' }}
