...
import { CaretRightOutlined, CommentOutlined, LoadingOutlined, PieChartOutlined, TeamOutlined } from '@ant-design/icons';
    ...
    <div  className='is-centered'>
      <div className=' container_agenda-information container-calendar2 is-three-fifths'>
        <Card style={{ padding:"0 !important"}} 
          title={
            <Row >
              <div style={{display:"flex",alignItems:"center",justifyContent: "center",  height:"7vh",width:"5vw"}}>
              <Col style={{marginLeft:"2vw"}} >
                <Row type="flex" style={{alignItems: 'center',justifyContent: "center"}}>{meetingState === 'ended_meeting_room' &&
                    (currentActivity.image || image_event) ?
                    <CommentOutlined style={{fontSize:"30px"}} /> :meetingState === '' || meetingState == null?  <CommentOutlined style={{fontSize:"30px"}} />: meetingState === 'closed_meeting_room'? <LoadingOutlined style={{fontSize:"30px"}} />: meetingState === 'ended_meeting_room' &&
                    currentActivity.video?<CaretRightOutlined style={{fontSize:"30px"}} /> : <img  style={{height:"4vh",width:"4vh"}} src={EnVivo} alt="React Logo" />}</Row>
                <Row style={{height:"2vh",fontSize:11 ,fontWeight:"normal"}}>{meetingState === 'ended_meeting_room' &&
                    (currentActivity.image || image_event) ?"Terminada": meetingState === '' || meetingState == null?" ":meetingState === 'closed_meeting_room'? "Por iniciar": meetingState === 'ended_meeting_room' &&
                    currentActivity.video? "Grabada":"En vivo"}</Row>
              </Col>
              </div>
              <Col style={{marginLeft:"2.5vw", marginTop:"1vh"}}>   
              <div style={{ height:"5vh"}}>
                  <Row style={{ height:"3.0vh"}}>{currentActivity.name} </Row>
                  <Row style={{ height:"2.5vh" ,fontSize:14 ,fontWeight:"normal"}}>{currentActivity?.space.name} </Row>
              </div>
            </Col>
          ...
          extra={
            ...
            event.id === '60061bfac8c0284c432069c8' ? (
              ...
              <>
              <div style={{ paddingRight:"2vw", height:"5vh", textAlign:"right !important",display:"block"}}><Col><Row style={{fontSize:14 ,height:"3.0vh", fontWeight:"normal",textAlign:"right",display:"block"}} ><p>{Moment(currentActivity.datetime_start).format('DD MMM YYYY')}</p> </Row>
              <Row style={{fontSize:14 ,fontWeight:"normal",textAlign:"right",display:"block",height:"2.5vh"}}>
                  <p>{Moment(currentActivity.datetime_start).format('h:mm a')} -{' '}
                  {Moment(currentActivity.datetime_end).format('h:mm a')}</p>
                 </Row></Col></div>
            ...
            <div className='is-block is-12 column is-paddingless'>
              ...
              {(meetingState === '' || meetingState == null) && option !== 'surveyDetalle' && option !== 'games' && (
                <div className='column is-centered mediaplayer'>
                  <img
                    className='activity_image' style={{width:"100%",height:"60vh"}}
