...
class DatosModal extends Component {
  ...
  changeFieldjustonebyattendee = () => {
    this.setState((prevState) => {
      return {
        info: {
          ...this.state.info,
          justonebyattendee: !prevState.info.justonebyattendee,
        },
      }
    })
  }
  ...
  changeFieldCheckVisibleByContacts = () => {
    this.setState((prevState) => {
      return {
        info: {
          ...this.state.info,
          visibleByContacts: !prevState.info.visibleByContacts,
        },
      }
    })
  }
  ...
  changeFieldCheckVisibleByAdmin = () => {
    this.setState((prevState) => {
      return {
        info: { ...this.state.info, visibleByAdmin: !prevState.info.visibleByAdmin },
      }
    })
  }
  ...
  handleKeyDown = (event) => {
    ...
    if (this.checkIfEnter(event)) {
      this.setState({
        ...
        info: {
          ...this.state.info,
          options: [...(this.state.info?.options || []), createOption(value)],
        },
      })
  ...
  handleKeyDownDependent = (event) => {
    ...
    if (this.checkIfEnter(event)) {
      ...
      this.setState((previous) => {
        ...
        newState.info = newState.info || {}
        newState.info.dependency = newState.info.dependency || {}
        newState.info.dependency.triggerValues =
          newState.info.dependency.triggerValues || []
  ...
  render() {
    const { inputValue, inputValueDependency, info, valid, loading, isDependent } =
      this.state
        ...
        >
          ...
          <Form.Item label="Posición / Orden" name="order_weight">
            <InputNumber
              ...
              onChange={(value) =>
                this.handleChange({ target: { name: 'order_weight', value: value } })
              }
            />
          </Form.Item>
          <Form.Item>
