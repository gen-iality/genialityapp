...
import { Card, Row, Spin, Col, Grid, Image, Typography, Divider } from 'antd'
...
const NoticiasDetails = (props) => {
  const { setVirtualConference, match, setTopBanner } = props
            ...
            <Col span={24}>
              <Card style={{ backgroundColor: '#F9F9F9' }} bordered={false}>
                <Meta
                  ...
                  description={
                    <div style={{ textAlign: 'center' }}>
                      <span>Actualizado: {noticia && noticia.updated_at}</span>
                    </div>
                  }
                />
                <br />
                <Card
                  ...
                  cover={
                    noticia &&
                    (noticia.linkYoutube === null ||
                    noticia.linkYoutube === '' ||
                    noticia.linkYoutube === undefined ? (
                      <Image
                        style={{
                          height: `${screens.xs ? '25vh' : '55vh'}`,
                          objectFit: 'cover',
                        }}
                        alt="imagen noticia"
                        src={
                          noticia && (noticia.image || noticia.picture)
                            ? noticia.image || noticia.picture
                            : imgNotFound
                        }
                      />
                    ) : (
                ...
                <NoticiaList
                  size={screens.xs ? 1 : screens.lg ? 3 : 2}
                  newId={noticia._id}
                />
              </Card>
            </Col>
...
const NoticiasDetailsConnect = connect(
  null,
  mapDispatchToProps,
)(withContext(withRouter(NoticiasDetails)))
