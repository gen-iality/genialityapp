...
function TimerAndMessageForTheNextQuestion(
  ...
  const timer = setInterval(() => {
    ...
    const mensaje = StateMessages(messageType, rankingPoints)
    ...
    mensaje.subTitle =
      secondsToGo > 0 ? mensaje_espera + ' ' + secondsToGo : mensaje_congelado
    setFeedbackMessage(mensaje)

    if (secondsToGo <= 0 && (!freezeGame || freezeGame === 'false')) {
