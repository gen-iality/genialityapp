...
export default (
  ...
  if (isRandomSurvey) {
    ...
    const sampleCount =
      randomSurveyCount === undefined
        ? surveyQuestions.length
        : Math.min(randomSurveyCount, surveyQuestions.length)
    ...
    if (sampleCount < surveyQuestions.length) {
      ...
      while (taken < sampleCount) {
        ...
        if (watchDog > surveyQuestions.length * 2) {
          console.error(
            'tanking random index has crashed and the loop has overflowed the survey questions length',
          )
          break
        }
      }
      ...
      newQuestions.push(
        ...surveyQuestions.filter((question: any, index: number) =>
          takenIndeces.includes(index),
        ),
      )
    } else {
