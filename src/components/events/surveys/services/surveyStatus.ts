 ...
 */
export const resetStatusByRestartAnswering = async (
  surveyId: string,
  userId: string,
): Promise<void> => {
  const firebaseRef = getRef(surveyId, userId)
...
export const addTriesNumber = async (
  ...
  try {
    await firebaseRef.set({ surveyCompleted, tried: newTried }, { merge: true })
    console.debug(
      `addTriesNumber (surveyId: ${surveyId}, userId: ${userId}, triedCount: ${triedCount}), maxTries: ${maxTries}, newTried: ${newTried}, surveyCompleted: ${surveyCompleted})`,
    )
  } catch (err) {
    console.error(`surveyId:${surveyId}, userId:${userId}`, err)
  }
...
export const addRightPoints = async (
  ...
  try {
    ...
    console.debug(
      `save survey status /votingStatusByUser/${userId}/surveyStatus/${surveyId}`,
      'value',
      acumulativePoints,
      '->',
      acumulativePoints + nextPoints,
    )
  } catch (err) {
    console.error(`surveyId:${surveyId}, userId:${userId}`, err)
  }
