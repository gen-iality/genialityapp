...
export const getUserProgressRef = (surveyId: string, userId: string) => {
  return getRef(surveyId).collection('userProgress').doc(userId)
}

export const getAnswersRef = (surveyId: string, userId: string) => {
  return getRef(surveyId).collection('answers').doc(userId)
}

export const getQuestionsRef = (surveyId: string, userId: string) => {
  return getRef(surveyId).collection('questions').doc(userId)
}
...
export const setCurrentPage = async (
  ...
  try {
    await firebaseRef.set({ currentPageNo }, { merge: true })
    console.debug(
...
export const initRealtime = (surveyId: string, onChange: (surveyData: any) => void) => {
  ...
  const unsubscribe = getRef(surveyId).onSnapshot((result) => {
