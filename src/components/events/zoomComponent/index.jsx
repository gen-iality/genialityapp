  ...
  async componentDidMount() {
    ...
    firestore
      ...
      .onSnapshot((response) => {
        ...
        this.setState({
          meeting_id: videoConference.meeting_id
            ? videoConference.meeting_id
          ...
          chat:
            videoConference.tabs && videoConference.tabs.chat
              ? videoConference.tabs.chat
              : false,
          surveys:
            videoConference.tabs && videoConference.tabs.surveys
              ? videoConference.tabs.surveys
              : false,
          games:
            videoConference.tabs && videoConference.tabs.games
              ? videoConference.tabs.games
              : false,
          attendees:
            videoConference.tabs && videoConference.tabs.attendees
          ...
          {this.state.event && platform === 'vimeo' && (
            <Row className="platform-vimeo" style={{ display: 'contents' }}>
              ...
              {this.state.contentDisplayed &&
                this.state.contentDisplayed == 'surveys' && (
                  <div style={{ width: '100%' }}>
                    <SurveyForm
                      event={event}
