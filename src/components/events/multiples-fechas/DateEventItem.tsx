...
export const DateEventItem = ({ date, onClick, handledDelete, canFinish, ...cardProps }: Props) => {
  const { dateFormat } = useDateFormat();
  console.log('canFinish',canFinish)
  return (
    <Card hoverable style={{ borderRadius: 10, height: '100%' }} {...cardProps}>
      <Row gutter={[8, 8]}>
        <Card.Meta
          avatar={
        ...
        <Space wrap align='center' style={{position: 'absolute', right: 20, top: 30}}>
          ...
          {canFinish && <Button danger type='default' icon={<IconsAntDesing.DeleteOutlined />} onClick={() => handledDelete(date.id)} />}
        </Space>
      </Row>
    </Card>
  );
