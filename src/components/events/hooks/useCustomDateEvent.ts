  ...
  const handleInterceptor = (values: DateObject | DateObject[] | null) => {
    if (Array.isArray(values)) {
      ...
      const newDateRangeList = datesSelected.map((date) => {
        console.log('dayToKey(new Date(date))', dayToKey(new Date(date)));
        const currentDataRange = dates.find((dataRange) => (dataRange.id = dayToKey(new Date(date))));

        console.log('currentDataRange', currentDataRange);
