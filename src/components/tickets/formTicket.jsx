...
class TicketsForm extends Component {
  ...
  componentDidMount() {
    ...
    const haspayments = !!this.props.cEvent.value.tickets.find(
      (item) => item.price !== '0',
    )
    ...
    const tickets = this.props.cEvent.value.tickets.map((ticket) => {
      ...
      const stage = !this.props.cEvent.value.stages
        ? null
        : this.props.cEvent.value.stages.find(
            (stage) => stage.stage_id === ticket.stage_id,
          )
      //Lista de opciones para el select
      ticket.options =
        stage && (stage.status === 'ended' || stage.status === 'notstarted')
          ? []
          : Array.from(Array(parseInt(ticket.max_per_person, 10) || 1)).map(
              (e, i) => i + 1,
            )
      return ticket
    })
  ...
  handleQuantity = (e) => {
    ...
    if (value === '0') this.removeTicket(name)
      ...
      const list = this.props.cEvent.value.seatsConfig
        ? [...this.state.ticketstoshow].map((i) => i._id)
        : []
      this.setState(
        { ticketsadded, disabledSelect: list.filter((i) => i !== name) },
        () => {
          this.renderSummary()
        },
      )
  ...
  submit = (seats) => {
    ...
    if (seats) {
      ...
      const quantity = this.state.summaryList
  ...
  async request(data) {
    ...
    try {
      ...
      const resp = await Actions.post(
        `/es/e/${this.props.cEvent.value._id}/checkout`,
        data,
      )
