...
const UserStatusAndMenu = (props: any) => {
	...
	let menu = !props.anonimususer ? (
		...
		<Menu>
			{!props.anonimususer ? (
				<Menu.Item key='user-status-menu-item-1-anonimususer' style={ItemStyle}>{`Bienvenido ${props.cUser?.value?.names}`}</Menu.Item>
			) : (
				<Menu.Item key='user-status-menu-item-2-anonimususer' danger icon={<LogoutIcon style={{ fontSize: '18px' }} />} onClick={() => showPropsConfirm()}>
					<FormattedMessage id='header.logout' defaultMessage='Salir' />
				</Menu.Item>
			)}
		</Menu>
