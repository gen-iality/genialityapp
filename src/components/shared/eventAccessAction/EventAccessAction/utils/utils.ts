...
const EXTERNAL_REDIRECT_COPYS: CopyStatus = {
  withURL: {
    title: 'Estás abandonando Magnetic',
...
const internalOrExternalEvent = ({ cEvent, history }: internalOrExternalEventInterface) => {
  ...
  const isArkMedEvent = !!organizationId
    ? ['6435b160705b239e5506c727', '6414c574211337f07601ae13'].includes(organizationId)
    : false;
  ...
  if (isArkMedEvent) {
    ...
    confirm({
      ...
      content: isArkMedEvent
        ? EXTERNAL_REDIRECT_COPYS[withURL].contentArkMed
        : EXTERNAL_REDIRECT_COPYS[withURL].content,
      onOk() {
...
export const assignStatusAccordingToAction = ({
  ...
  const already_registered_event = intl.formatMessage({
    id: 'already_registered_event',
    defaultMessage: 'Ya estás inscrito en el evento',
  });
  ...
  const has_been_invited_event = intl.formatMessage({
    id: 'has_been_invited_event',
    defaultMessage: 'Has sido invitado a este evento',
  });
